...
def parse_tax_return(pdf_path: Path) -> dict:
    ...
    result["근로소득금액"] = _income_amt(full, r'근\s*로\s*소\s*득\s*금\s*액')
    result["연금소득금액"] = _income_amt(full, r'연\s*금\s*소\s*득\s*금\s*액')
    result["기타소득금액"] = _income_amt(full, r'기\s*타\s*소\s*득\s*금\s*액')
    ...
    # ── 연금계좌세액공제 납입액 (세액공제 명세서 276번) ─────────────────
    # 신고서: "연 금 저 축 276 6,000,000 (15%) 720,000" 형태
    # 안내문과 동일하게 납입액 기준으로 파싱
    m_pens = re.search(r'연\s*금\s*저\s*축\s*276\s+([\d,]+)', full)
    if not m_pens:
        m_pens = re.search(r'연금계좌\s*세액공제[^\n]*\n[^\n]*276\s+([\d,]+)', full)
    result["연금계좌납입액"] = int(m_pens.group(1).replace(",", "")) if m_pens else None
...
def parse_anneam(pdf_path: Path) -> dict:
    ...
    result["연금계좌세액공제"] = _amt(r'연금계좌세액공제\s*([\d,]+)원')   # 납입액 기준
...
def cross_verify(
    ...
    # ── H. 소득종류 누락 검증 ────────────────────────────────────────
    # 근로(단일)/근로(복수) 통합: 둘 중 하나라도 O면 근로소득 대상
    근로_O = (str(안내문.get("근로(단일)", "X")).strip() == "O" or
               str(안내문.get("근로(복수)", "X")).strip() == "O")

    소득_체크 = [
        (근로_O,                                              "근로소득금액", "근로소득"),
        (str(안내문.get("연금", "X")).strip() == "O",        "연금소득금액", "연금소득"),
        (str(안내문.get("기타", "X")).strip() == "O",        "기타소득금액", "기타소득"),
        (str(안내문.get("이자", "X")).strip() == "O",        "이자소득금액", "이자소득"),
        (str(안내문.get("배당", "X")).strip() == "O",        "배당소득금액", "배당소득"),
    ]
    for ann_O, 신고서_key, 소득명 in 소득_체크:
    ...
    # ── J. 연금계좌세액공제 체크 ────────────────────────────────────
    # 안내문에 연금계좌세액공제 납입액이 있으면 신고서와 비교
    ...
    if ann_연금납입 is not None and ann_연금납입 > 0:   # 0원이면 체크 불필요
        if 신고서_연금납입 is None:
            add("공제 체크", "연금계좌세액공제",
                "[안내문] 납입액", ann_연금납입,
                "[신고서] 연금저축(276)", "파싱불가",
                "warn", f"안내문 납입액 {ann_연금납입:,}원 — 신고서 적용 여부 수동 확인", diff=None)
        elif ann_연금납입 == 신고서_연금납입:
            add("공제 체크", "연금계좌세액공제",
                "[안내문] 납입액", ann_연금납입,
                "[신고서] 연금저축(276)", 신고서_연금납입,
                "pass", f"연금계좌납입액 일치 — {ann_연금납입:,}원",
                diff=0)
        else:
            add("공제 체크", "연금계좌세액공제",
                "[안내문] 납입액", ann_연금납입,
                "[신고서] 연금저축(276)", 신고서_연금납입,
                "warn", f"납입액 불일치 — 안내문 {ann_연금납입:,}원 / 신고서 {신고서_연금납입:,}원",
                diff=ann_연금납입 - 신고서_연금납입)
